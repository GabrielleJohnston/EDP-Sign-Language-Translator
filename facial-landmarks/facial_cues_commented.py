...
################################################################################
    ...
    # number (name of the variables) refers to number on coordinate map - see file on github
    three = []
    twenty = []
    thirty_eight = []
    thirty_four = []
    ...
    while fvs.more():
        ...
        if frame is not None:
            ...
            counter = 0
            for j in shape:
                # create an array containing the x and y coordinates of point 3 on the landmark map
                if counter == 2:
                    three = shape[counter]
                # create an array containing the x and y coordinates of point 20 on the landmark map
                if (counter == 19):
                    twenty = shape[counter]
                # create an array containing the x and y coordinates of point 28 on the landmark map
                if counter == 27:
                    twenty_eight = shape[counter]
                # create an array containing the x and y coordinates of point 34 on the landmark map
                if counter == 33:
            ...
            # finding the difference between point 20 and 38, i.e. distance between the eye and eyebrow
            d_difference = abs(twenty[1] - thirty_eight[1])
            # append that to an array called d_differences
            d_differences.append(d_difference)

            # finding the difference between point 34 and 28, i.e. distance between the top of the nose and bottom of the nose
            ...
            # finding the difference between point 34 and 3, i.e. distance between the bottom of the nose and the side of the face
    ...
    # finding the ratio between the eye/eyebrow distance and top-of-the-nose/bottom-of-the-nose distance
    # this results in distance invariance as the eye-eyebrow distance is normalised by top-of-the-nose/bottom-of-the-nose distance
    # top-of-the-nose/bottom-of-the-nose distance would scale up and down depending on the distance between user and the camera
    # this assumes that the user cannot move their nose which is quite a valid assumption
    ...
    # this can be due to poor environmental setting and imperfect tracking of the program used
    sum = 0
    ratios_average = []
    ...
    # k is a threshold value - achieved from numbers of testings and may vary between people
    k = 0.05
    eyebrowMovement = 'neutral'
    # counter1 refers to frowning, counter2 to eyebrows being raised
    for i in range(0, len(ratios_average)):
        x = False
        # compare the average ratio with the reference ratio (ratio at the start of the program)
        # if the value is lower than the reference value, frowning is detected
        if ratios_average[i] <= (live_ratios[0] - k/4):
            counter1 = counter1 + 1
            x = True
        # if the value is higher than the reference value, eyebrow raising is detected
        if ratios_average[i] >= (live_ratios[0] + k/2):
            counter2 = counter2 + 1
            x = True
        # 3 consecutive frowning/eyebrow raising detected required to conclude that the user indeed frown/ raising eyebrow
        # this is equivalent to frowning/eyebrow raised for an approximately 1 second
        if x == False:
            if counter1 > 3:
    ...
    # find the change of the distance between the nose and side of the face in time
    ...
    # the 2 and -2 are threshold values, acquired from testings this is to prevent the program from
    # considering face twitching as face turning
    # simplify that to just '-' for negative change, '+' for positive change and 'o' for no change
    # store that in array called delta_array
    ...
    # delta is negative when the face is turning left i.e. the distance between the nose and the left side of the face decreases
    # delta is positive when the face is turning right i.e. the distance between the nose and the left side of the face increases
    # hence, when there is a change in sign within a small period of time face is turned
    headshake = False
